        ...
        + (if ($tally.review_stopped // []) | length > 0 then
             ...
             | "**Why the loop stopped:** "
               ...
               + (if $attributed < $tally.documents then
                    " and NOT the window: the other \($tally.documents - $attributed) were delivered before this breakdown was recorded, so these counts — and the `cap`/`converged`/`truncated` split in the next paragraph, which is drawn from them — are out of \($attributed) and do not scale up to the rate. Nothing after that paragraph is: `unresolved_severity` and `unfinished_page_rate` below are both over all \($tally.documents) documents, so subtract the floor from the rate as it stands and do not rescale it first. They are not a sixth kind of exit"
