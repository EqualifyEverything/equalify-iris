...
(if .rule then
  ...
  else
    {
      ...
      "unresolved": ("Over the last \($tally.window_days) days, **\(.value | pct)% of \($tally.documents) documents** finished the review loop with issues the loop could not resolve.\n\nThe threshold is \(.threshold | pct)% of documents.\n\nThese are barriers the Reader Agent found, reported, and that the loop then shipped anyway — listed in each session's `unresolved.md`. Some floor here is inherent, because a source document can be genuinely ambiguous, so what this issue actually asks is whether the current rate is that floor or a pattern. The three lines below are what answer it (#264), and each is absent on a deployment that does not record it — zeroes beside a non-zero rate read as a contradiction.\n\n"
        ...
        + (if ($tally.review_stopped // []) | length > 0 then
             ...
             + ". One per delivered document, so these sum to \($tally.documents) and a shortfall is documents delivered before the breakdown existed. `cap` is the only one raising `defaults.max_review_iterations` can help — read it beside `mean_rounds` = \($tally.mean_rounds). `converged` is the editor having been shown the issues and answered \"no change\", which the loop treats as final ON PURPOSE (the next round is the same request about the same body), so its remedy is `agents/copy_editor.md` or `agents/reader.md` and never more rounds. `truncated` is the output ceiling, `unread` a review that could not read part of what it judged.\n\n"
           else "" end)
