# Sprint #246: a recommended approach for every step

This is the model-selection sprint's report — how every figure in
**[docs/cost.md](cost.md)** was reached, which model belongs on which step and why, the errata against
...
**`copy_editor`'s 33.1% is a per-agent share, and the per-step figure for the same round is 16.8%.
Both are right.** Worth writing down because the two were read as a contradiction and the reconciliation
lived only in a thread. On `runs-bystep-now` the two steps named after the editor come to `edit` 4.1%
plus `edit_section` 12.7% = **16.8%**, which is why a per-step reading of that round cannot find 33.1%.
The agent also bills at `table_join` and for whole-document attempts that failed, and those add up:
$0.8010 (`edit`) + $2.4673 (`edit_section`) + $1.4734 (failed) = $4.7417, plus $1.6701 at
`table_join` = **$6.4118 of $19.3951 = 33.1%**, exactly as #317 publishes it. So 16.8% is a floor on
...
(§6). The step-by-step breakdown of that $0.1071 is [docs/cost.md](cost.md).
...
- **A share table that did not sum to 100% and nobody checked it** — four agent shares summing to
  ...
  cost tables in §3 here and in docs/cost.md.
...
- **A shortened model id printed a count instead of a name.** `split(".").pop()` on
  `moonshotai.kimi-k2.5` printed a bare `5`, and a two-model round was published as mixing five models
  on one step. A helper for exactly this bug already existed, with a comment describing exactly this
...
node bystep.mjs        runs-bystep-now              # §1's per-step shares, and the 33.1% reconciliation
```
