...
* `editor_headings_gated_rate` — share of documents where at least one correction round had **blocks
  ...
  **measurable rather than only stated** (#375): both paths compute the same reading and log it as
  ...
  where the demotions are not. Which of the two it is was not answerable from this number before and is
  not answerable from it now — it is answerable from that line, which is the point of having it.
