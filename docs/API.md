...
* `editor_truncated_rate` — share of documents where a correction round's **response** hit the
  ...
  finished arriving is applied, and the round covers the document up to the last block it named
  (`editor_salvaged`). Only the part it never reached is asked for again, **a section at a time** —
...
* `editor_truncated_lost_rate` — share of documents where neither route covered the whole body. The
  ...
  nothing (`editor_salvaged`, `closed`/`reached`). Where there is a remainder, either the prefix
...
it, so nothing was left to ask for at all.
...
```json
{
  ...
  "errors": [],
  "verification": {
    "pages_verified": 25, "pages_unjudged": 2, "pages_skipped_blank": 2, "verify_failed": 13, "corrections": 14,
