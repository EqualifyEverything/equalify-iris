...
* `lint_error_rate` — share of documents whose lint pass **errored** instead of running. Recorded
  ...
  attribute whose *name* no valid markup produces used to take the whole rule set offline through
  the selector engine, so the lint drops those from its own copy of the document before axe walks
  it and counts them on the line instead (`malformed_attributes`, and `lint_debris` where there is
  no `assembly` line — #257). A run with that count set and no `lint_error` is a document that used
