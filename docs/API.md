...
lint on the **assembled** document, where every id has been renamed, so the finding names an id no
page agent ever wrote. So the duplicate is checked in code on the page's own fragment, off the
