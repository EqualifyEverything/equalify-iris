...
* `table_no_body_rate` — share of documents delivered with at least one table that has a header
  block and no rows under it. A screen reader announces the table, reads its caption and every
  column header, and there is nothing in it. Measured on the **parsed** tree, unlike the rate above,
