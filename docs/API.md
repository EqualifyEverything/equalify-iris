...
* `editor_headings_gated_rate` — share of documents where at least one correction round had **blocks
  ...
  **document** rather than per round — a document whose editor demotes a heading on three rounds
  ...
  seated inside the form), or when the block that dropped the heading changed its own words at all.
  Re-seating either would print those words twice, so that round is refused whole and logs
  `discarded: "headings_lost"` with `headings_dropped` and no `headings_reverted` beside it.

  **What a 0 here does not mean.** It is not evidence that no round demotes a heading, so it is not on
  its own a reason to retire the guard. Two application paths never compute this reading at all: the
  sectioned round (`editor_section_failed`'s path, which is the loop's **last** round, so nothing looks
  at its output again) and the whole-body reply, both of which check only the size floor. A demotion on
  either is applied and delivered, and this rate stays 0. A 0 is therefore consistent with a guard that
  never fires *and* with a guard that is looking somewhere the demotions are not; distinguishing them
  needs `navigation_lost` read off the run logs of documents that took a sectioned round, not this
  number.
...
here by the doubt-word veto (`blank_vetoed`). Its evidence is the `page_blank` line and `pages_blank`.
