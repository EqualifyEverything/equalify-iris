...
* `editor_truncated_rate` — share of documents where a correction round's **response** hit the
  ...
  could exceed a fixed `max_tokens`. Such a round is re-made **a section at a time** — the body is cut at top-level
...
| `page_correction_failed` | A self-correction's model call threw (`page`, `image`, `trigger`, `problems`, `error`, `truncated`, `output_cap`, `chars_kept`), so the page keeps the version it already had — the extraction that succeeded, verified minutes earlier. It costs the **correction**, not the page: before this, the error propagated out of the page's own task and the run logged `page_extraction_failed` and shipped a `@page-failed` marker for a page it still had, which also named a stage that had worked (issue #171). Paired with `page_corrected` `result: "failed"`. Every error class is survivable here, not only a ceiling — a throttle, a stall and a truncation all leave behind a page good enough to have been worth correcting — and nothing is retried, because a correction truncating because the *page* is large will truncate again for a second full ceiling of output. `truncated: true` says the model wrote an essay where a page was asked for, which is worth reading beside `page_verify_failed`'s problem list. Read it with `output_cap`, which is the ceiling this call asked for and is sized from the page it was correcting (`correctionOutputCeiling`, issue #285): a truncation at that number is a runaway the cap made cheap, and its remedy is the multiple at the call site rather than `providers.*.max_tokens` — which is already higher and raising it would only buy a larger discarded reply. A `truncated: true` line whose `output_cap` equals the deployment's ceiling is the older, uncapped shape, and that one does have a configuration remedy. `chars_kept` is the size of the fragment that ships, and the page the cap was derived from, so the pair can be checked against each other. The fidelity problems the correction was asked to fix are still unfixed and still on record — keeping the page is not a claim that it was right. |
