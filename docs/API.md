...
```json
{
  ...
  "verification": {
    ...
    "verify_kinds": { "content_missing": 5, "content_wrong": 2, "structure_wrong": 6,
                      "a11y_only": 3, "alt_quality": 4, "untagged": 1 },
...
what made `verify_failed` unreadable as an accuracy signal. Not a partition, for the same reason
...
things cannot outweigh six pages naming one each. `untagged` is the page count that keeps the rest
honest: a page whose problems carried no kind — an older log, an agent file whose contract predates
the kinds, a model that answered in plain strings — is in `verify_failed` and in no kind bucket, and
a split read without `untagged` beside it is a split of the tagged share presented as the whole run.
A page appears in both when some of its problems were tagged and some were not. Paired with
...
says which remedy applies. It costs the same as the ceiling does (that page, not the run) and for
