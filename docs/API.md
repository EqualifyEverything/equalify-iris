...
* `editor_headings_gated_rate` — share of documents where at least one correction round was thrown
  away because applying it would have left the delivered body with **fewer headings than it had**,
  every word still there (`discarded: "headings_lost"` on an `editor_patch` line, #331). The one
  number in this tally that goes **up** when a guard is working: the round it counts did not ship, so
  the document it is counted on kept its outline. It is a rate of *attempts*, not of damage, and per
  **document** rather than per round — a document whose editor demotes a heading on three rounds
  ...
  every round spends the whole budget and the document is delivered as it entered, with its issues in
  `@unresolved`. That is a statement about `agents/copy-editor.md` or the model behind it, not about
  the budget: raising `max_review_iterations` buys more of the same round. A rise with no `cap` beside
  it is the cheap case, one retried round per document, and can also be the two known false
  positives — a heading correctly re-expressed as a `<label>`, `<caption>`, `<dt>` or `<th>`, or a
  reprinted title dropped in a way that left the prose no shorter.
