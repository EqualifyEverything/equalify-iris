...
naming what was wrong, one self-correction pass was bought, and it did not replace the page. So the
markup delivered for them is the markup that failed the check, byte for byte.
...
<!-- @page-uncorrected 5, 31
  ...
  is the markup that was rejected. ...
-->
```
...
The correction ends without replacing the page in exactly four ways, and for the delivered bytes they
are one fact, so the marker does not distinguish them. The log does: `page_correction_failed` is the
call that threw, and `page_corrected`'s `result` is `empty` (it answered with no HTML), `identical`
(it answered with the page it was given) or `rejected` (its answer came back at under a quarter of
that page's size and was refused as a deletion). `page_verify_failed` on the same image says what was
wrong. The set itself is on `extraction_complete.uncorrected` (or `reextract_complete.uncorrected`)
