...
**Severity is decided by reachability, not by category.** The reviewer has a ranked list of what to *look at* — accessibility correctness of the output (§7.7), upstream side effects and filing identity (§7.13), auth and tokens (§9.1), provider routing and cost (§10.3), general correctness, failing checks, missing tests, and the PR's own template contract. That list does not determine the verdict. Blocking requires that a real user, a real request, or CI reach the defect on input the code accepts today; a defect that is real and correctly reasoned but unreachable is posted as a note on an **approval**, stating what would have to change to reach it. Each blocking finding must name the input that reaches it, and a review whose every finding is latent is required to be an approval.
...
**Known coverage gap, disclosed rather than hidden.** `claude-code-action` refuses to run when its own workflow file differs from the copy on the default branch, so **a PR that modifies `code-review.yml` gets no automated review** while every other check on it still goes green. Between PR #43 and PR #63 a second workflow covered that gap, running on `pull_request_target` so it executed the default branch's definition; it never once produced a model-written review in six runs, and it was the repo's only `pull_request_target` + `id-token: write` job — a PR-triggered job holding secrets. It was deleted, accepting the gap over the privilege. The self-skip branch now writes a step-summary block and an Actions warning naming what a human must check on such a diff: whether it lets PR-authored code run with secrets, widens `permissions:`, or interpolates `github.event.*` into a `run:` block.
...
**This does not make merge automatic.** The reviewer's verdict is advisory: `main` requires a pull request but deliberately does not require this check, and §7.13's gatekeeper is still a human maintainer. Three kinds of PR get no review at all, and requiring the check would mishandle each of them differently rather than uniformly: a PR touching only `paths-ignore`d files never triggers the workflow, so the check never reports and the PR is unmergeable; a PR editing `code-review.yml` runs the job to completion with the fallback-review step and the verify step both gated off, so the check reports **success** with no review behind it (observed on PR #70); and a fork PR's job is skipped by its own `if:`, which should satisfy a required check the same vacuous way — that last one is GitHub's documented handling of a skipped job, not a measurement. The gate on the fallback step is what makes the second case silent: without it the fallback would post a `--request-changes` review as `github-actions[bot]`, and the check would be green *with* a verdict rather than green with none. (The Claude step produces nothing on such a PR either, but by `claude-code-action` refusing to run when its own workflow differs from the copy on the default branch — not by anything in this file, and it is `continue-on-error` regardless.) One case blocks a mergeable PR forever and two put a green tick on an unreviewed one — the second failure is the worse of the two, because it is indistinguishable from a real pass exactly where a human read matters most. What the workflow changes is what that human is reading — a diff already checked, already searched for the eight categories above, and already carrying a stated accessibility impact — not whether they read it.
...
- **An issue that any open PR already addresses is not eligible, and if every open issue is covered the run does nothing.** Restricting this to the workflow's own `iris-auto/` branches would leave the gap open from the one direction that matters most: a contributor's PR closing an issue is the case where a duplicate wastes *two* people's work, and it is invisible to a branch-prefix check. Coverage is read from three signals — GitHub's `closingIssuesReferences`, an `issue-<n>` fragment in the head branch, and a bare `#<n>` in the title or body — and the last of those is deliberately loose. It is genuinely noisy: this repo's PR bodies cite issue and PR numbers freely, and "related to #5 but does not fix it" is textually identical to a fix. Two things make it the right default anyway. Every reference is intersected with the set of currently-open issues, which discards PR numbers and closed issues without judgement; and the failure modes are asymmetric — a false positive delays an issue by one run and prints the PR that caused it in the run summary, while a false negative spends a review slot on a duplicate, which is the specific cost this section exists to avoid. A manual dispatch overrides it, with a warning naming the PR.
