...
**The job runs the checks itself and hands the model their output.** Before the model is invoked, one step runs `npm ci`, `tsc --noEmit`, the unit suite, `./test/e2e.sh`, `actionlint` (version-pinned, and its download verified against a recorded SHA-256 — an unverified linter installer would be a supply-chain path into a job holding `id-token: write`) and `shellcheck` over `.github/scripts/*.sh` (a workflow's own logic, kept out of the YAML only because a `run:` block may not exceed 21000 characters, and read by nothing else), then assembles a context file: PR metadata, a "Check summary" of those six results, the full diff, and the full source of files that are new or substantially rewritten. The model reads that file first and is told not to re-run the checks. This exists so the reviewer reasons about *measured* results rather than predicted ones — a claim that a test fails is either quoted from the summary or it is not made.
