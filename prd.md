...
**This does not make merge automatic.** The reviewer's verdict is advisory: `main` has no branch protection, and §7.13's gatekeeper is still a human maintainer. What the workflow changes is what that human is reading — a diff already checked, already searched for the eight categories above, and already carrying a stated accessibility impact — not whether they read it.
