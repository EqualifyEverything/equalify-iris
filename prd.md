...
**A field invited to hold prose is a field that quotes the contract back, and the reply reader has to survive that.** `extractJson` returns the LAST readable object in a reply, which is what rescues a model that drafts before it answers (§7.11, issue #170) — so a `notes` string containing an unescaped `{ "faithful": true, "problems": [] }` ends the reply with a second object, and that one carries the decision flag. Read as the verdict it is a **confident pass on a page the verifier has just rejected**: `ok` true, no problems, no `unjudged` marker, and a plain `page_verify_ok` line, which is the one outcome `pages_unjudged` cannot see. The repair rule ended the `notes` value at the quote before each `:` (`repairedSpan`, `src/util/json.ts`), so the real envelope stopped parsing and the quoted fragment was all that read. Two changes close it and neither is a stricter judge. First, a reply that is nothing but its object — first character to last, which is what this contract asks for — is read with the colon rule confined to keys, where JSON is the only thing that puts a colon after a string, and that reading is preferred only where **two** conditions hold: the ordinary walk did not already close on the reply's last character, and the reading's own strings are **self-contained** — every `{` inside them closing inside the same string. Every limit there is measured rather than cautious. Applied to every candidate in the walk the narrow rule changes 14 of the 4,100 agent replies in the bench logs and loses on all 14, because a Reader verdict that quotes `{"html":"…` in its prose gets a value-string that never closes and swallows the verdict behind it, returning one issue in place of five. And its own failure case is a string the position tracker reads as a value where JSON meant a key, which is what a draft abandoned mid-string and restarted inline does: there the narrow reading returns the abandoned prose with `{"html": "` glued to the front of the page. Both gates refuse that shape and it took both. A restart the walk can read **whole** leaves nothing after it for a second reading to recover, because its closing brace is the reply's own — that, and not "a restart is always the last thing in the reply", is the claim the code rests on; a reply that restarts, closes the object and then adds a sentence is refused by the whole-reply attempt's own span check instead. The brace test catches a restart the walk could read only in part, since a model that *quotes* an object writes its closing brace while a model that *abandons* one leaves that brace to the object it restarted; on its own it is not sufficient, because a single `}` in the restarted page content — a code listing, template syntax, a math brace — rebalances the abandoned string, and an earlier revision of this amendment claimed the test held whatever the abandoned string contained. A decoy the model quoted sits inside a field of an envelope that closes after it, so it fails neither gate. Discarding a reading costs nothing, which is what lets both tests be strict: the answer then is the ordinary walk's, which is the answer before any of this, so this repair can only ever be inert or right on a reply it was not written for. Over the same 4,100 replies the result is byte-identical to the reading before it. Second, and because a fenced or preamble-prefixed reply is still beyond any one-pass reader, **`verifyAgentOutput` treats a reply that does not carry both `faithful` and `accessible` as booleans as not a verdict at all** — 1,342 of 1,342 readable verify replies in those logs carry both, so the check costs nothing measurable, and what it converts is a silent pass into a page counted in `pages_unjudged`. It is not free in principle, and the direction it costs is worth naming: a reply that says `faithful: false`, names its problems and omits `accessible` used to buy a correction pass and is now a page nothing judged, so that defect ships counted but uncorrected. No reply in the logs has that shape, and the trade is deliberate — a pass that never happened is visible in `pages_unjudged`, and a pass that did is not visible anywhere. The prompt asks for no quoted JSON in `notes` as well; that is the cheapest of the three and the only one that reaches a fenced reply, and it is not relied on alone. **A whole CLASS of decoy defeats all three, and it is stated here at its real width rather than left to be discovered**: a quoted decoy containing **any string value at all**. The `"` opening it follows a `:` and the `"` closing it is followed by `,`, `}` or `]`, and both are terminators under every reading in `src/util/json.ts` — they have to be, since that is how the repair reads the last field of every reply that needed repairing — so the real `notes` value ends inside the quoted sentence, no whole-reply object survives to be preferred, and the quoted contract carries both flags as booleans and passes the verdict gate. That is a pass on a page the verifier rejected, unchanged from before this amendment, and the only layer that reaches it is the prompt clause, which is why the clause is not decoration. What this amendment's repair fixes is the decoy with **no** string values in it, which is the shape #339 produced. An earlier revision of this paragraph called the residual "one shape … a decoy quoting an *empty* string, `{ …, "notes": "" }`", which named the mechanism correctly and the width wrongly, in the more expensive direction: the empty string is the minimal instance, not the trigger, and whoever next tried to close the class would have made that pin pass and believed it closed. Both the empty and a non-empty instance are pinned as assertions of the current wrong answer in `test/envelope-as-content.test.ts`, to be inverted together by whatever closes the class.

**Amended (v1.10): two clauses of the page contract decided the same image twice, in opposite directions, and a third had no case at all for a legend whose symbol is ink.** Both were found by re-reading rounds already paid for, both cost `$0` to fix, and neither is a cost lever — what they corrupt is the document and, in the first case, a measurement the deployment prices a step with. The **cover collision** (issue #351): the IMAGES rule exempts *a graphic whose content this page ALSO carries in full beside it* and separately enumerates *a cover whose appearance is itself the content* as informative, and a cover satisfies both at once — its title, banner, publisher and date are transcribed alongside, and its appearance is the content. Nothing ordered them, so the clause that won was whichever the reader reached first, and on one 100-page document every arm's first pass demanded the image by the enumeration while the sampled recheck reversed one of them by the exemption, each quoting this contract accurately; three arms produced three outcomes on the same page. That lands on the §7.4 v1.6 recheck sample rather than on the bill — the second look is the measurement pass, not a second correction, so no extra call is made — and it lands one-directionally, since a "still failing" verdict which no correction can satisfy without breaking the other clause can only push the sampled pass rate down. The rule now says informative wins, bounds the exemption to *a graphic the page repeats BESIDE it, never a graphic the page IS*, and says what the description is for — the appearance, which the transcription does not carry — which is also what reconciles it with the redundancy clause that follows.

**The graphical key is the sharper of the two, because the contract's own prohibition was what suppressed the description** (issue #347). ABBREVIATIONS AND KEYS asks for a legend as *a `<dl>` of symbol and meaning* and works the case out only for a symbol that has a textual form — the ■ or ▶‖ on a machine's keys, named from the page. A shading key's symbol is an area of ink with no words printed for it anywhere, so that `<dl>` cannot be completed in the page's own words, and describing the ink reads as the invented expansion the same rule's first clause forbids. On the nine legend-bearing figure pages of a 100-page document the extraction read the key correctly and the verify-and-correct pair — **63.7%–76.0% of what those pages cost** — then removed it: two of three verify calls cited that prohibition against a described `<dd>` and the compliant correction deleted it, so the **only three serious accessibility violations in that arm's entire output were created by the repair rather than found by it**, and the sole arm axe failed on these pages was the sole arm that gave the legend any structure at all. Both halves are now stated — the described ink is the `<dt>` and the printed wording its `<dd>`, in words and never in a `style` attribute — and the counterpart is in `agents/feedback.md`, which is the half that binds, since the extraction got this right unprompted and the verifier talked it out of it. Two further clauses come from measuring the source image rather than reading it: a key's shades **run in the printer's order and not in the order its entries are listed** (measured on one legend: 26, then 176, then 143 — non-monotonic, and both agents assumed a ramp and wrote the assumption into the markup as fact), and where two swatches are indistinguishable in the reproduction that is said rather than resolved by dividing items between them, because the extraction's own hedge — *"none visibly distinct from medium in this reproduction"* — was the correct answer on a page whose two light bands are 33 luminance units apart under a 113-unit lighting vignette, and the verifier overturned it with a false certainty the correction paid **48.6% of that page** to install. **Every clause here names where its answer goes, and that is not a stylistic point in this rule.** A first revision of the entry-count clause said only *"say how many entries the key prints"*, and in the case the rule is about there is no `<dt>` or `<dd>` for a count to sit in, so the nearest home a model finds is a sentence beside the `<dl>` — prose the page does not print, which this same rule forbids two paragraphs above and which a verifier reads as invented text, reopening the exact delete loop the amendment exists to close. The count therefore goes in the `alt` where the key is described there (a description is scaffolding the prompt names as sanctioned) and in the `log` either way. The verify-side shield is bounded for the mirror-image reason: it refuses a report **as invented text** rather than refusing a report at all, so a *wrongly* named tone stays reportable as `content_wrong`, and it says a hedge is falsifiable — a verifier that can tell two swatches apart says which is which — because a protected hedge nobody can check would make an unclassified item the cheapest answer the page agent has, and the trade this amendment takes is a gap the page *admits* over a confident assignment to the wrong band, not a gap nobody may question. How often that hedge ships is not measured today and is worth a `first_read`-style count. Finally, `agents/feedback.md` now says that a problem's **reason** is part of the licence and not commentary on it: the same page's third problem called the legend's own printed two-line heading *"invented text not present as a legend label"*, and the correction deleted it, where "a heading glued to the first label" would have licensed moving those words. A right finding with a wrong reason buys the wrong repair. None of this is visible downstream — the corrected page is well-formed, specific and false, and every automated gate passes it, which is an argument for a measured oracle rather than for a different model. What is explicitly **not** changed is the recheck's non-binding sampling design (§7.4 v1.6): the defect here is not that the repair went unchecked, it is that the finding the repair executed was wrong.

**Amended (v1.11): the page's own arithmetic is a check on the picture, and the verifier was asked to lengthen a list the page caps** (issue #353). On a figure whose printed subtitle reads *"**Eight** of the **Twelve** States That Shift…"*, the extraction transcribed that subtitle verbatim into the `<figcaption>` and, ten lines above it in the same fragment, enumerated **40** states as above-average. The verify pass had both strings, quoted the legend's labels verbatim in its own first problem — and its third problem asserted one further state into the category, which the correction obeyed, delivering **41**. Measuring the sheet decides it against them: three fills are distinguishable where the legend names two, and their dispersion splits **8** flat near-black from **4** dotted with no member between them and a 4.3x gap, so the printed arithmetic and the measured partition agree exactly and everything else is base map — the state the verifier added carries no category at all. That measurement is also the argument for the cheap rule rather than a better one: the legend's own swatch reads 100 against fills of 32–42 and 163–186, and neither distance clears the panel's 55-unit lighting gradient, so **the caption was the only sound decoder on this page** and a reader working from the reproduction alone cannot classify a single state. The page contract now says that a count the page prints about its own picture is transcribed evidence, that a description enumerating that category's members is counted against it before it is emitted, that the **list** gives way rather than the count, and — because a rule that says only "make them agree" licenses both wrong repairs — that the list is neither padded to the number nor trimmed to fit, with the mismatch reported in the `alt` and the `log` and never as a sentence beside the figure. That puts the printed count in two places, immediately above the redundancy rule that forbids the description repeating what the words beside the image already say, so the exclusion is **stated rather than derived**: that rule is about the *name* of the thing pictured, which a caption announces on its own, while a number is transcription where the page prints it and the bound on the list where the `alt` carries it. The audience that would derive it wrongly is the same verifier that read a described swatch as invention and deleted it (#347). `agents/feedback.md` carries the same comparison, ordered ahead of anything that turns on the ink because it is decidable where the ink may not be, and with the asymmetry that actually stops this defect — but bounded at the **count** rather than at lengthening. A list may be sent back to be shortened and never taken **past** the number the page prints, since adding a member to a category the page itself caps is the one repair that cannot be right. "Never lengthened" was the first wording and it was too wide: a list naming nine where the page prints twelve is three members missing, a real `content_missing` finding, and an unconditional ban leaves it with no repair path at all — so that direction stays reportable, with the printed number quoted in the problem so the repair inherits the page's own bound, and with the other route back to the same defect closed (a verifier that cannot say *which* members are missing says the list is short of the count and names none). Four figure pages have now been graded against their ink and the verifier was wrong on all four, so what is being bounded is not a stray error.

**The same issue's second half is the pipeline being told what the page agent already said about itself.** `suggested_agent` lets the page agent ask for a specialist; `dispatchSpecialist` routes it and logs the outcome, and nothing else read it. In one 100-page round across two page-model arms there were **7 requests on 5 pages under 6 distinct names, every one a map specialist, 0 dispatched** — and those 5 pages were every page in the round whose verdict turned on reading ink, including all four graded wrong. On the page above the agent asked for help producing *"a structured data table of each state's classification"*, got the classification wrong, and the verify step — never told — added to it. So the unmet request is now carried into the verify message (`specialistCaution`), placed after the cached prefix so a per-page string cannot cost every other page in the document its cache read, and `agents/feedback.md` says what it narrows: **Unmet is a different question from dispatched, and `dispatchSpecialist` now answers it separately** — the two disagree on four of that function's six exits, and keying the caution on `dispatched` mislabelled all four. Three of the four were silent and the fourth was the expensive one, so the count is stated with that split: no content, a throw and an unmerged fragment all say `dispatched: true` and so said nothing about a request that went unmet, while the standard-type decline says false and so shipped a caution that was actively wrong, on the commonest suggestion shape there is. `dispatched` answers whether a suggestion is already covered and so need not be filed as a new-agent issue; the caution asks whether specialist content reached the HTML the verifier is about to judge. A specialist that ran and returned nothing of its type, one that threw (dispatch is non-blocking by design, so the page arrives looking exactly like a page that never asked), and one whose fragment would not merge are all dispatched **and** unmet, and each leaves the page agent's own unaided attempt in front of the verifier — verbatim the condition being reported. A **standard** type is the case in the other direction: no specialist ran, but the request was *answered*, by the v1.2 policy that makes the general pass that type's intended handler rather than a fallback for it, so it carries no caution. That carve-out is a judgement and is priced as one: standard types are the commonest suggestion shape there is, cautioning them would narrow what the verifier may assert on most of the pages that ask for anything — on a table page, decline to say a cell reads wrongly — and it would buy nothing measured, since 0 of the 7 requests behind this issue were standard types. So the dispatch returns the reason a request went unmet as a phrase rather than a flag, because "no agent of that name was available" is simply false about a specialist that ran and failed, and the four are not interchangeable to a verifier deciding how much of a page to doubt. The two model-written strings are flattened and clipped on the way in (a `reason` carrying a fence of its own would restructure the message after the `html` block it annotates). And `agents/feedback.md` says what the caution narrows: the verifier may say a reading is unsupported and ask for it to be hedged, scoped or removed, and may not supply a replacement reading of its own. **The flag is not treated as a detector and the prompt does not depend on it being one.** It is page-level rather than per-arm — 4 of those 7 requests came from the other arm's reader on the same images, so a run sees only its own — and it missed two of the round's hard pages outright, which is why the licence bound is stated unconditionally and the flag only says where it has already gone wrong for nothing. Six spellings of one concept also mean a name-matched registry will keep missing this; whether to add a map specialist at all is **not** decided here (that is #323's question), and nothing in this amendment changes routing, dispatch or the §7.4 v1.6 sampling design.
