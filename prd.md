...
This is nearer §7.9's original wording than the whole-body request that displaced it, and what it changes is the blast radius of a bad reply as much as the length of a good one. A refusal now costs the block it was about instead of the round: a replacement that leaves an element open or closes one that was never opened, a block number the document does not have, a repeated one, an unreadable entry, an echoed marker — each keeps that block's original text while the rest of the reply is applied, and each is counted. Two shapes are not applied in part, and the log says which of them it saw: a reply where nothing could be used at all, and a reply holding one of those refusals beside a block that gave content up — because this contract makes a *move* a pair of edits, so taking the source half and refusing the landing half deletes content that no later pass can miss. Both hand back the body that entered the round, which is a retry and not a v1.2 convergence, since the editor has not said anything usable about this document; an empty `edits` array, by contrast, is a usable round that changed nothing and converges exactly as an identical whole body did. #174's size floor is unchanged and reads the body those blocks assemble into rather than any one replacement. The v1.3 and v1.4 paths remain for what the contract cannot fix: a reply is still capable of hitting the ceiling — carrying a single top-level node bigger than it, the largest one measured across that corpus being ~24,000 tokens, or answering with the whole document anyway — and when it does, the round is still cut into sections and then still contained. A whole-body reply is read rather than refused, because refusing it spends the round and a model that falls back to a familiar shape under load would spend every round of the run; it is logged (`editor_whole_body`), and the markers it copied back out of the document it was shown are stripped and counted, since delivering them would write Iris's own request scaffolding into the HTML — and would compound, a comment being a top-level node that becomes a block in its own right next round.
