...
// still runs against a bare checkout.
...
async function renderPage(
  ...
  agent: AgentSpec,
  img: InputImage,
): Promise<{ html: string; log: string }> {
  const user =
  ...
  return { html: parsed?.html ?? stripFences(res.text), log: parsed?.log ?? "" };
}
...
// Feedback Agent is unavailable or unsure.
export async function runExtraction(ctx: PipelineContext): Promise<ExtractionResult> {
  ...
  for (const img of ctx.images) {
    const { html, log } = await renderPage(ctx, pageAgent, img);
    let innerHtml = html;
    let logNote = log;
