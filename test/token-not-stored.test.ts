...
// deployment's identity, so every request to `/v1/me` or `/v1/sessions` 500s — the four
// ungated routes keep answering — and the operator is left reading a SQLite constraint
// on a column no current build writes.
test("a pre-existing database with a token column is refused at startup", async () => {
  ...
  try {
    ...
    assert.throws(
      ...
      "an older database was adopted silently — every /v1/me and /v1/sessions request would 500 with a SQL error",
    );
