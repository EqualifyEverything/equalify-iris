...
- **A page the document has no content for is reported once, not once per chunk.** Two kinds of
  ...
  was asked about both, once per chunk, per round: `runReader` gives every chunk the same page
  index so the bytes can be cached, a lost page's entry there was the failure's own marker and a
  blank page's was an empty line, and every call that saw one reported it in its own wording. On
  the round that filed issue #188 that was 6 of one document's 26 unresolved issues for a single
  page, and the multiplier is chunks × iterations, so a longer document inflated it further —
  `unresolved` is the `iris:unresolved` signal behind `/v1/stats`'s `clean_rate` and
  `/v1/quality`'s `unresolved_rate`. Both entries now say what the page is and that it is not an
  issue to report, `READER_SYSTEM` says the same with the reasons, and a round's repeats are
  reduced to one report per page (`reader_page_reports_deduped`). The FIRST report is kept rather
  than all of them dropped: an issue attributed entirely to pages with no content can only be about
  the absence, but that attribution is the Reader's, so a misattributed real issue must not vanish
  without a trace. An issue naming any page that *does* have content is never touched.
